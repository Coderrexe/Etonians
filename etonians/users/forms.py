...
class RegistrationForm(FlaskForm):
    username = StringField(
        label="Nickname",
        validators=[DataRequired(), Length(min=2, max=20, message="Must be between 2 and 20 characters long.")]
    )
    email = EmailField(
        label="Email",
        validators=[DataRequired(), Email(message="Email is not valid.")]
    )
    password = PasswordField(
        label="Password",
        validators=[DataRequired(), Length(min=8, message="Must be at least 8 characters long.")]
    )
    ...
    def validate_username(self, username):
        ...
        if user:
            raise ValidationError(
                message="This username is already used by another account. Please choose another one."
            )
...
class UpdateAccountForm(FlaskForm):
    ...
    # validate_username and validate_email functions will be automatically called
    def validate_username(self, username):
        # function will be only carried out if the user decides to actually change his username
...
class UpdateAccountPasswordForm(FlaskForm):
    old_password = PasswordField(
        label="Your Old Password", validators=[DataRequired()]
    )
